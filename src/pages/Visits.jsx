...
export default function Visits() {
  ...
  const [visitType, setVisitType] = useState('check-in');
  ...
  const findAvailableSlots = async () => {
    ...
    for (let i = 0; i < 30; i++) {
      ...
      for (const staffMember of availableStaff) {
        ...
        const canDoMorning = await checkTimeSlotFeasibility(selectedProperty, staffVisitsOnDate, 'morning', dateStr);
        const canDoAfternoon = await checkTimeSlotFeasibility(selectedProperty, staffVisitsOnDate, 'afternoon', dateStr);
  ...
  const checkTimeSlotFeasibility = async (selectedProperty, staffVisitsOnDate, timeSlot, dateStr) => {
    ...
    for (const visit of staffVisitsOnDate) {
      ...
      try {
        ...
        if (visit.scheduled_time === 'morning' && timeSlot === 'morning') return false;
        if (visit.scheduled_time === 'afternoon' && timeSlot === 'afternoon') return false;
        if (visit.scheduled_time === 'morning' && timeSlot === 'afternoon') {
          if (totalTimeNeeded > 240) return false;
        }
        if (visit.scheduled_time === 'afternoon' && timeSlot === 'morning') return false;
      } catch (error) {
  ...
  const handleCreateVisit = async () => {
    ...
    try {
      ...
      if (visitType === 'check-in') {
        if (!editingId && checkForScheduledVisit()) return;
        ...
        if (editingId) {
          ...
          if (newVisit.is_recurring && newVisit.recurrence_end_date) {
            ...
            while (currentDate < endDate) {
              if (newVisit.recurrence_frequency === 'daily') currentDate = addDays(currentDate, 1);
              else if (newVisit.recurrence_frequency === 'weekly') currentDate = addWeeks(currentDate, 1);
              else if (newVisit.recurrence_frequency === 'bi_weekly') currentDate = addWeeks(currentDate, 2);
              else if (newVisit.recurrence_frequency === 'monthly') currentDate = addMonths(currentDate, 1);
              
              if (currentDate <= endDate) dates.push(format(currentDate, 'yyyy-MM-dd'));
            }
          }

          await base44.entities.Visit.bulkCreate(dates.map(date => ({ ...visitData, scheduled_date: date })));
        }
      } else {
        const visitData = {
          tenant_id: companyId,
  ...
  const columns = [
        ...
        return (
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="font-medium text-slate-900 truncate">{property?.name || property?.address}</p>
    ...
    },
    {
      ...
      cell: (visit) => (
        <div>
          ...
          {visit.scheduled_time && <p className="text-sm text-slate-500">{visit.scheduled_time}</p>}
        </div>
      ),
    ...
    {
      header: 'Visit Type',
      cell: (visit) => <StatusBadge status={visit.visit_type} />,
      className: 'hidden md:table-cell'
    },
    {
      header: 'Assigned',
      cell: (visit) => <span className="text-slate-600">{visit.assigned_to_name || '—'}</span>,
    ...
    {
      header: 'Details',
      cell: (visit) => <span className="text-sm text-slate-600">{getVisitDetails(visit)}</span>,
      className: 'hidden sm:table-cell'
    },
    {
      header: '',
  ...
  return (
    <div>
      ...
      <div className="mb-6 flex gap-2 justify-end">
        <Button
          onClick={() => setShowVisitTypeDialog(true)}
      ...
      {/* Visit Type Selection Dialog (Record Visit) */}
            ...
            {!editingId && (
              <div className="flex gap-2 mb-4">
                <Button
                  ...
                  onClick={() => { setScheduleMode('manual'); setSearchResults([]); }}
                ...
                <Button
                  ...
                  onClick={() => { setScheduleMode('search'); setSearchResults([]); }}
            ...
            <div className="space-y-4 py-4">
                ...
                >
                  ...
                  <SelectContent>
                    {properties.length === 0 ? (
                      <div className="p-2">
                        <Button
                          onClick={() => { setShowNewDialog(false); navigate(createPageUrl('PropertyForm')); }}
              ...
              {scheduleMode === 'search' && !editingId && (
                <div className="space-y-4">
                  ...
                  <Button onClick={findAvailableSlots} disabled={!newVisit.property_id} className="w-full bg-blue-600 hover:bg-blue-700">
                    ...
                    <div className="mt-4 space-y-2">
                      {searchResults.map((result, idx) => (
                        <button
                          ...
                          onClick={() => {
                            if (visitType === 'check-in') {
                              setNewVisit(prev => ({ ...prev, scheduled_date: result.date, scheduled_time: result.time, assigned_to: result.staffEmail }));
                            } else {
                              setNewVisit(prev => ({ ...prev, followup_due_date: result.date, followup_due_time: result.time, assigned_to: result.staffEmail }));
                            }
                            setScheduleMode('manual');
                          }}
                ...
                <>
                  {visitType === 'check-in' ? (
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <Select value={newVisit.scheduled_time} onValueChange={(value) => setNewVisit(prev => ({ ...prev, scheduled_time: value }))}>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        <Label>Time Block</Label>
                        <Select value={newVisit.followup_due_time} onValueChange={(value) => setNewVisit(prev => ({ ...prev, followup_due_time: value }))}>
                          <SelectTrigger><SelectValue placeholder="Select time" /></SelectTrigger>
                          <SelectContent>
                            <SelectItem value="morning">Morning (8am-12pm)</SelectItem>
                            <SelectItem value="afternoon">Afternoon (12pm-4pm)</SelectItem>
                  ...
                  {visitType === 'check-in' ? (
                    <>
                      ...
                      {newVisit.checkin_type === 'other' && (
                        <div className="space-y-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                          <div>
                            ...
                            <Input placeholder="e.g., Annual Maintenance Check" value={newVisit.custom_name} onChange={(e) => setNewVisit(prev => ({ ...prev, custom_name: e.target.value }))} />
                          </div>
                          <div>
                            <Label>What was checked?</Label>
                            <Textarea placeholder="Describe what was checked..." value={newVisit.checkin_details} onChange={(e) => setNewVisit(prev => ({ ...prev, checkin_details: e.target.value }))} rows={2} />
                          </div>
                      ...
                      {newVisit.checkin_type === 'customer_called_in' && (
                        <div className="space-y-3 p-3 bg-purple-50 border border-purple-200 rounded-lg">
                          <div>
                            ...
                            <Textarea placeholder="Describe what the client requested..." value={newVisit.checkin_details} onChange={(e) => setNewVisit(prev => ({ ...prev, checkin_details: e.target.value }))} rows={2} />
                          </div>
                      ...
                      {newVisit.checkin_type === 'drop_in' && (
                        <div className="space-y-3 p-3 bg-green-50 border border-green-200 rounded-lg">
                          <div>
                            <Label>Check-In Name</Label>
                            <Input placeholder="e.g., Quick Property Check" value={newVisit.custom_name} onChange={(e) => setNewVisit(prev => ({ ...prev, custom_name: e.target.value }))} />
                          </div>
                    ...
                    <div className="space-y-4">
                      <div>
                        <Label>Title *</Label>
                        <Input placeholder="e.g., Fix roof leak" value={newVisit.followup_title} onChange={(e) => setNewVisit(prev => ({ ...prev, followup_title: e.target.value }))} />
                      </div>
                      <div>
                        <Label>Priority</Label>
                        <Select value={newVisit.followup_priority} onValueChange={(value) => setNewVisit(prev => ({ ...prev, followup_priority: value }))}>
                          <SelectTrigger><SelectValue /></SelectTrigger>
                          <SelectContent>
                      ...
                      <div>
                        ...
                        <Textarea placeholder="Describe the follow-up details..." value={newVisit.followup_description} onChange={(e) => setNewVisit(prev => ({ ...prev, followup_description: e.target.value }))} rows={2} />
                      </div>
                    </div>
                  )}
                  ...
                  {visitType === 'check-in' && templates.length > 0 && !['other', 'customer_called_in', 'drop_in'].includes(newVisit.checkin_type) && (
                    <div>
                      ...
                      <Select value={newVisit.template_id} onValueChange={(value) => setNewVisit(prev => ({ ...prev, template_id: value }))}>
                  ...
                  {visitType === 'check-in' && !['other', 'customer_called_in', 'drop_in'].includes(newVisit.checkin_type) && (
                    <div className="flex items-center justify-between py-2">
                      ...
                      <Switch checked={newVisit.is_recurring} onCheckedChange={(checked) => setNewVisit(prev => ({ ...prev, is_recurring: checked }))} />
                    </div>
                  )}
                  ...
                  {visitType === 'check-in' && newVisit.is_recurring && (
                    <div className="space-y-4 p-4 bg-slate-50 rounded-lg">
                      <div>
                        ...
                        <Select value={newVisit.recurrence_frequency} onValueChange={(value) => setNewVisit(prev => ({ ...prev, recurrence_frequency: value }))}>
                          <SelectTrigger><SelectValue /></SelectTrigger>
                          <SelectContent>
      ...
      <Dialog open={showReplaceDialog} onOpenChange={setShowReplaceDialog}>
        <DialogContent className="max-w-md rounded-2xl p-0 bg-white/90 backdrop-blur-xl border border-white/30 shadow-2xl">
          ...
          <DialogFooter className="gap-2 px-6 py-5">
            <Button variant="outline" onClick={() => handleReplaceScheduled(false)} disabled={creating}>Keep Both</Button>
            <Button onClick={() => handleReplaceScheduled(true)} disabled={creating} className="bg-slate-900 hover:bg-slate-800">

...
export default function VisitDetail() {
  ...
  const loadVisit = async () => {
    ...
    try {
      const visitData = await base44.entities.Visit.filter({ id });
      
      if (visitData.length > 0) {
        const vis = visitData[0];
        setVisit(vis);
        setEditData({
          scheduled_date: vis.scheduled_date,
          scheduled_time: vis.scheduled_time || '',
          type: vis.visit_type === 'check-in' ? vis.checkin_type : vis.followup_type,
          assigned_to: vis.assigned_to || ''
        });
  ...
  const handleUpdateVisit = async () => {
    ...
    try {
      ...
      if (visit.visit_type === 'check-in') {
        updateData.checkin_type = editData.type;
      } else {
        updateData.followup_type = editData.type;
      }
      
      await base44.entities.Visit.update(visit.id, updateData);
  ...
  const handleNotifyClient = async () => {
    ...
    await base44.integrations.Core.SendEmail({
      to: client.email,
      subject: `Visit Report - ${property?.name || property?.address}`,
      body: `
...
Date: ${format(new Date(visit.scheduled_date), 'MMMM d, yyyy')}
Status: ${visit.overall_status === 'all_clear' ? 'All Clear' : 'Issues Found'}

${visit.summary_notes ? `Summary: ${visit.summary_notes}` : ''}
...
Your Property Management Team
  ...
  const totalItems = visit.checklist_data?.reduce((sum, section) => sum + (section.items?.length || 0), 0) || 0;
  const completedItems = visit.checklist_data?.reduce((sum, section) => 
    sum + (section.items?.filter(item => item.status)?.length || 0), 0) || 0;
  const flaggedItems = visit.checklist_data?.reduce((sum, section) => 
    ...
    <div className="max-w-5xl mx-auto">
      ...
      >
        <div className="flex gap-3">
          ...
          {visit.status === 'completed' && !visit.client_notified && client && (
        ...
        <Card className={`mb-6 ${
          ...
          <CardContent className="flex items-center gap-4 py-4">
            {visit.overall_status === 'all_clear' ? (
              ...
              <AlertTriangle className={`h-8 w-8 ${visit.overall_status === 'urgent' ? 'text-red-600' : 'text-amber-600'}`} />
            )}
            <div>
              ...
              {visit.summary_notes && (
                <p className="text-sm mt-1 opacity-80">{visit.summary_notes}</p>
              )}
            </div>
          </CardContent>
        </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              <div className="flex justify-between items-center">
                ...
                <StatusBadge status={visit.visit_type === 'check-in' ? visit.checkin_type : visit.followup_type} />
              </div>
              <div className="flex justify-between items-center">
                ...
                {editingDate ? (
                  <Input
                    ...
                    onBlur={async () => {
                      if (editData.scheduled_date !== visit.scheduled_date) {
                        setUpdating(true);
                        try {
                          await base44.entities.Visit.update(visit.id, {
                            scheduled_date: editData.scheduled_date
                          });
                          setVisit(prev => ({ ...prev, scheduled_date: editData.scheduled_date }));
                        } catch (error) {
                          console.error('Error updating date:', error);
                          setEditData(prev => ({ ...prev, scheduled_date: visit.scheduled_date }));
                        } finally {
                          setUpdating(false);
              ...
              {visit.assigned_to_name && (
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{visit.assigned_to_name}</span>
                </div>
              )}
              {visit.completed_at && (
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">
                    {format(new Date(visit.completed_at), 'MMM d, h:mm a')}
                  </span>
                </div>
              )}
            </CardContent>
            ...
            <Card>
              <CardContent className="pt-6 space-y-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-2xl font-bold">{visit.photo_count || 0}</p>
          ...
          {visit.status === 'completed' && (
            <Card>
              <CardContent className="pt-6">
                ...
                {visit.client_notified ? (
                  <p className="text-sm text-emerald-600">
                    ✓ Sent {visit.client_notified_at && format(new Date(visit.client_notified_at), 'MMM d, h:mm a')}
                  </p>
                ) : (
          ...
          <Tabs defaultValue="checklist">
            <TabsList className="w-full justify-start mb-4">
              ...
              {visit.issues_found?.length > 0 && (
                <TabsTrigger value="issues">Issues ({visit.issues_found.length})</TabsTrigger>
              )}
            </TabsList>
            ...
            <TabsContent value="checklist">
              <Card>
                <CardContent className="pt-6">
                  {!visit.checklist_data || visit.checklist_data.length === 0 ? (
                    ...
                    <div className="space-y-6">
                      {visit.checklist_data.map((section, sectionIndex) => (
            ...
            <TabsContent value="photos">
              <Card>
                <CardContent className="pt-6">
                  {(() => {
                    const allPhotos = visit.checklist_data?.flatMap(section => 
                ...
                <Card>
                  <CardContent className="pt-6">
                    ...
                    <div className="space-y-4">
                      {visit.issues_found.map((issue, index) => (
